<?php
/**
 * Kemnaker Proposal Controller
 * Handles CRUD operations for Kemnaker Proposals
 */

class KemnakerProposal {
    private PDO $db;
    ...
    /**
     * GET /kemnakerProposals - Get all Kemnaker proposals
     */
    public function findAll(): void {
        try {
            ...
            logError("FindAll KemnakerProposal error: " . $e->getMessage());
            errorResponse('Failed to fetch Kemnaker proposals', 500);
        }
    }
    
    /**
     * GET /kemnakerProposals/{id} - Get Kemnaker proposal by ID
     */
    public function findById(string $id): void {
        try {
            ...
            if (!$proposal) {
                errorResponse('Kemnaker proposal not found', 404);
                return;
            }
            
            successResponse($proposal);
        } catch (PDOException $e) {
            logError("FindById KemnakerProposal error: " . $e->getMessage());
            errorResponse('Failed to fetch Kemnaker proposal', 500);
        }
    }
    
    /**
     * POST /kemnakerProposals - Create new Kemnaker proposal
     */
    public function create(): void {
        try {
            ...
            $materiTitle = $data['materi_title'] ?? null;
            $materiTeori = $data['materi_teori'] ?? null;
            $materiPraktik = $data['materi_praktik'] ?? null;
            ...
            $stmt->execute([
                ...
                'materi_teori' => $materiTeori,
                'materi_praktik' => $materiPraktik,
            ...
            successResponse([
                ...
                'title' => $title,
                'proposal_category' => $proposalCategory
            ], 'Kemnaker proposal created successfully', 201);
            
        } catch (PDOException $e) {
            logError("Create KemnakerProposal error: " . $e->getMessage());
            errorResponse('Failed to create Kemnaker proposal', 500);
        }
    }
    
    /**
     * PUT /kemnakerProposals/{id} - Update Kemnaker proposal
     */
    public function update(string $id): void {
        try {
            ...
            $stmt = $this->db->prepare("SELECT id FROM kemnaker_proposals WHERE id = :id AND deleted_at IS NULL");
            $stmt->execute(['id' => $id]);
            ...
            $fieldMap = [
                ...
                'condition' => '`condition`',
                'facility' => 'facility',
                'materi_title' => 'materi_title',
                'materi_teori' => 'materi_teori',
                'materi_praktik' => 'materi_praktik',
            ...
            $sql = "UPDATE kemnaker_proposals SET " . implode(', ', $fields) . " WHERE id = :id";
            
            $stmt = $this->db->prepare($sql);
            $stmt->execute($params);
            
            successResponse(['id' => $id], 'Kemnaker proposal updated successfully');
            
        } catch (PDOException $e) {
            logError("Update KemnakerProposal error: " . $e->getMessage());
            errorResponse('Failed to update Kemnaker proposal', 500);
        }
    }
    
    /**
     * DELETE /kemnakerProposals/{id} - Delete Kemnaker proposal (soft delete)
     */
    public function delete(string $id): void {
        try {
            ...
            $stmt = $this->db->prepare("UPDATE kemnaker_proposals SET deleted_at = NOW() WHERE id = :id");
            $stmt->execute(['id' => $id]);
            
            successResponse(null, 'Kemnaker proposal deleted successfully');
            
        } catch (PDOException $e) {
            logError("Delete KemnakerProposal error: " . $e->getMessage());
            errorResponse('Failed to delete Kemnaker proposal', 500);
        }
    }
}
